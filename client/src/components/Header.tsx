      ...
      <div className="container mx-auto px-4 py-2 flex flex-col">
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            ...
            />
            <h1 className="font-montserrat font-bold text-xl text-black">Pedidos Fácil</h1>
          </div>
        ...
        <nav className="flex gap-3 mt-4">
          {categorias.map(cat => (
            <a
              key={cat.id}
              href={`#${cat.id}`}
              className="px-3 py-1 rounded-full bg-gray-100 text-gray-800 hover:bg-[#af1a2d] hover:text-white transition-colors"
            >
              {cat.nome}
            </a>
          ))}
